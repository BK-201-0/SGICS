...
class Model(object):
    ...
    def predict(self, batch, step, update=True, mode='train', ex=None):
        ...
        if mode == 'train':
            ...
            code_state, code_funcs, file_names, urls = build_batch(batch, self.network, ex=ex)
            loss_value = None
            output = {
...
# Building phase
def build_batch(batch, network, ex=None):
    network.train(False)
    with torch.no_grad():
        # code_state, code_funcs, file_names, urls = cal_code_features(network, batch)
        code_state, code_funcs, file_names, urls = in_cal_code_features(network, batch, ex)
...
def pad_tensor(tensor, target_size, dim, padding_value=0):
    current_size = tensor.size(dim)
    if current_size >= target_size:
        return tensor
    padding_sizes = [0] * (tensor.dim() * 2)
    padding_sizes[-(dim * 2 + 1)] = target_size - current_size
    return F.pad(tensor, pad=padding_sizes, value=padding_value)

def in_cal_code_features(network, ex, ex1):
    batch_size = ex['batch_size']
    code_graphs = ex['code_graphs']
    if network.message_function == 'edge_mm':
        code_edge_vec = code_graphs['edge_features']
    ...
    print(f"Shape of local_code_state: {local_code_state.shape}")

    doc_graphs = ex1['doc_graphs']
    doc_words = ex1['targets']
    if network.message_function == 'edge_mm':
        doc_edge_vec = doc_graphs['edge_features']
    else:
        doc_edge_vec = network.edge_embed(doc_graphs['edge_features'])
    ...
    print(f"Shape of local_doc_state: {local_doc_state.shape}")

    target_size = max(local_code_state.size(0), local_doc_state.size(0))
    local_code_state_padded = pad_tensor(local_code_state, target_size, dim=0)
    global_code_state_paded = pad_tensor(global_code_state, target_size, dim=0)

    # biggnn+biggnn+mha
    if network.code_info_type in ['all']:
        src_state = torch.cat([local_code_state_padded, local_doc_state], dim=-1)
        # src_state + local_doc_state
        src_state = torch.cat([src_state, global_code_state_paded], dim=-1)
    elif network.code_info_type in ['global']:
        src_state = global_code_state
    else:
        src_state = local_code_state

    src_state = network.global_att(src_state, src_state, src_state)

    print(f"Shape of src_state: {src_state.shape}")

    # src_state = self.network.global_att(src_state, src_state, src_state)
    return src_state.detach().cpu().numpy(), ex['code_func'], ex['file_names'], ex['urls']