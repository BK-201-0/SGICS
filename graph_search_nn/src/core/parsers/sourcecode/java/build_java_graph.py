...
environ['JAVA_HOME'] = '/data/hugang/Anaconda/envs/gsn_jjy'
environ['PATH'] += '/data/hugang/Anaconda/envs/gsn_jjy/bin/'

dir_path = os.path.dirname(os.path.realpath(__file__))

EXTRACTOR_JAR = os.path.join(dir_path, 'features-javac-extractor-1.0.0-SNAPSHOT-jar-with-dependencies.jar')
DOT_JAR = os.path.join(dir_path, 'features-javac-dot-1.0.0-SNAPSHOT-jar-with-dependencies.jar')

RAW_FILE = os.path.join('/data/hugang/JjyCode/GraphSearchNet/CodeSearchNet/java/final/jsonl')
JAVA_BASE = os.path.join('/data/hugang/JjyCode/GraphSearchNet/CodeSearchNet/java_dedupe_definitions_v2.pkl')
JAVA_BASE_DIR = os.path.join('/data/hugang/JjyCode/GraphSearchNet/CodeSearchNet/java_base1')
BASE_GRAPH_FILE = os.path.join('/data/hugang/JjyCode/GraphSearchNet/CodeSearchNet/java_base_gz')
...
def build_java_base_graphs():
    ...
    results = parallel_process(definitions, build_single_graph, args=(JAVA_BASE_DIR, ), n_cores=8)
    succeed = 0
    for result in results:
        if result:
            succeed += 1
...
def build_single_graph(sample, java_func_dir):
    try:
        ...
        if os.path.exists(wrapped_file_path + '.proto'):
            ...
            # if os.path.exists(wrapped_file_path + '.dot'):
            #     dot_to_json(wrapped_file_path)
            return True
        return False
    except:
        return False
...
# def dot_to_json(wrapped_file_path):
#     dot_file = os.path.join(wrapped_file_path + '.dot')
...
#             'type': node.get_attributes()
#         })
#
#     for edge in graph.get_edges():
#         edges.append({
#             'sourceId': edge.get_source(),
#             'destinationId': edge.get_destination(),
#             'type': edge.get_attributes()
#         })
#
#     graph_json = {
#         'node': nodes,
#         'edge': edges,
#         'type': graph.get_attributes()
#     }
#
#     json_file = os.path.join(wrapped_file_path + '.json')
#
...
def generate_proto_file(wrapped_file_path):
    ...
    command = '/data/hugang/Anaconda/envs/gsn_jjy/bin/javac -cp %s -Xplugin:FeaturePlugin %s' % (EXTRACTOR_JAR, wrapped_file_path)
    ...
    #     print("标准输出:\n", stdout.decode('utf-8'))
    # else:
    #     print(f"命令执行失败，返回码为：{p1.returncode}")
    #     print("错误输出:\n", stderr.decode('utf-8'))



def generate_dot_json_file(wrapped_file_path):
    command = '/data/hugang/Anaconda/envs/gsn_jjy/bin/java -jar %s -i %s -o %s -j %s' % (DOT_JAR,
...
#nohup python build_java_graph.py > out.txt 2>&1 &