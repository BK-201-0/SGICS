...
from ..layers.gattention import *
# from ..layers.dcmhattention import *
...
class Graph2Search(nn.Module):
    ...
    def __init__(self, config, word_embedding, word_vocab):
        ...
        # self.global_sequence_att = DCMHAttention(config['batch_size'], self.heads, self.graph_hidden_size, self.device)
        # self.global_code_att = DCMHAttention(config['batch_size'], self.heads, self.graph_hidden_size, self.device)

        self.global_att = GMultiHeadedAttention(self.heads, self.graph_hidden_size * 3, config, config['word_dropout'])
    ...
    def forward(self, ex, criterion=None):
        ...
        if self.code_info_type in ['all']:
            ...
            src_state = torch.cat([src_state, global_code_state], dim=-1)
        elif self.code_info_type in ['global']:
            src_state = global_code_state
        else:
            src_state = local_code_state
