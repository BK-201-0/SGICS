...
class ModelHandler(object):
    ...
    def __init__(self, config):
        ...
        # if config['num_gpu'] > 1:
        #     print(f"Let's use {config['num_gpu']} GPUs!")
        #     self.model.network = torch.nn.DataParallel(self.model.network, device_ids=list(range(config['num_gpu'])))
    ...
    def _run_epoch(self, data_loader, training=True, verbose=10, client=None,  ex=None):
        ...
        for step in range(data_loader.get_num_batch()):
            ...
            res = self.model.predict(x_batch, step, update=training, mode=mode, ex=ex)
